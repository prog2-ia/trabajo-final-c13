...
El desarrollo de este sistema ha integrado un conjunto de funcionalidades avanzadas que complementan la arquitectura orientada a objetos y elevan la calidad del código hacia estándares profesionales. Entre ellas destaca la sobrecarga de operadores, mediante la cual se han implementado métodos mágicos como _add, __iadd, __gt, __lt, __eq_ y _getitem_. Esta técnica permite operar con las entidades del dominio de manera intuitiva y expresiva, facilitando la fusión de listas de reproducción, la adición eficiente de canciones, la comparación por duración y la validación de igualdad, todo ello siguiendo las convenciones idiomáticas de Python y reduciendo la verbosidad del código sin sacrificar legibilidad.

Para garantizar una ejecución robusta y predecible, se ha diseñado un sistema de excepciones personalizadas que heredan de la clase base Exception. Estas clases específicas se lanzan cuando se detectan inconsistencias lógicas o violaciones de las reglas de negocio, como registros duplicados, valores fuera de rango o intentos de acceso a colecciones vacías. Su captura mediante bloques try/except en la interfaz de usuario asegura que el programa gestione casos límite de forma controlada, evitando terminaciones abruptas y proporcionando retroalimentación clara que guía al usuario hacia la resolución del problema sin interrumpir el flujo de trabajo.

La persistencia de datos se aborda mediante dos mecanismos complementarios que equilibran interoperabilidad y eficiencia. Por un lado, la exportación a ficheros de texto utiliza un formato delimitado para almacenar las colecciones principales, facilitando la auditoría manual y la compatibilidad con herramientas externas, mientras que un registro de actividad en modo append preserva el historial de operaciones críticas. Por otro lado, la serialización binaria a través del módulo pickle permite guardar y restaurar el estado completo del objeto principal de la biblioteca en un único archivo, garantizando una recuperación rápida y fiel de todas las relaciones entre objetos, contadores y configuraciones al reiniciar la aplicación. Esta dualidad se apoya en una gestión automática de directorios que verifica y crea las rutas necesarias antes de cualquier operación de entrada y salida.

Finalmente, aunque Python se caracteriza por ser un lenguaje de tipado dinámico donde el intérprete verifica los tipos en tiempo de ejecución y se rige por el principio de duck typing, este proyecto incorpora sugerencia de tipos para introducir una capa de verificación estática y documentación explícita. Se han utilizado anotaciones como str, int, list[Any], Optional[T] y Union para definir contratos claros en variables, parámetros y retornos, aprovechando módulos como typing y collections.abc para manejar iterables y estructuras complejas. Para validar estas anotaciones, se integra Mypy, un comprobador estático opcional que analiza el código sin ejecutarlo, detectando incompatibilidades antes del runtime. Mediante su ejecución en modo estricto, se garantiza la coherencia de tipos en toda la base de código, mientras que el uso estratégico de directivas de ignorancia en los setters de propiedades permite armonizar el encapsulamiento estricto con la verificación estática, eliminando falsos positivos y consolidando un desarrollo más mantenible, seguro y alineado con las buenas prácticas modernas de Python.
